...
const newAboutUsUpdate = asyncHandler(async (req, res) => {
  ...
  const content = section.aboutUs.find((aboutUs) =>
  aboutUs._id.equals(imageId)
  );
  if (!content) {
  ...
  const newAboutUSALL = asyncHandler(async (req, res) => {
    try {
      const aboutUs = await MainModel.find({}, 'sections').lean();

      const allaboutUs = aboutUs.reduce((images, doc) => {
        const sections = doc.sections || []; 
        for (const section of sections) {
          const aboutUs = section.aboutUs || []; 
          images = images.concat(aboutUs);
        }
        return images;
      }, []);
      
      res.status(200).json(allaboutUs);
    } catch (error) {
      res.status(500).json({ error: 'Internal Server Error' });
    }
  });

  const newAboutUSUDelate = asyncHandler(async (req, res) => {
    const imageId = req.params._id;
    
    try {
      const mainDocument = await MainModel.findOne({ "sections.aboutUs._id": imageId });
      ...
      const section = mainDocument.sections.find(section =>
        section.aboutUs.some(img => img._id.equals(imageId))
      );
      ...
      section.aboutUs = section.aboutUs.filter(img => !img._id.equals(imageId));
      await mainDocument.save();
  ...
  const newAboutUSAdd = asyncHandler(async (req, res) => {
    const mainId = req.params.mainId;
    const sectionId = req.params.sectionId;
